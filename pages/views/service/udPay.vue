...
<script>
	export default {
		...
		methods: {
			initData() {
				this.$http.get('/v1/check/recharge/status/udcz').then(res => {
			...
			initMidform(e) {
				this.$http.get("/v1/show/uc/recharge",{params:{pcid: 3}}).then(res => {
					if (res.data.code == 200) {
						this.midList = res.data.data.data.cardMid
						for (let i = 0; i < this.midList.length; i++) {
							this.midArr.push(this.midList[i].point)
						}
						this.disList = res.data.data.data.payDis
						this.playCheck(0)
					} else {
						uni.showToast({
						    icon: 'none',
			...
			totalMoney() {
				if (this.midCheck > -1) {
					this.money = this.formatMoney(Number(this.midList[this.midCheck].point) * this.num * this.discount * 0.1)
				}
			},
			handNum(e) {
				if (e < 0 && this.num <= 0) {
			...
			applyTo(e) {
				this.$http.post("/v1/uc/recharge", {
					...
					let msg = "充值面额:" + this.mid  + ",充值数量:" + this.num + ",充值金额：" + this.money +",充值账号：" + this.account+ "."
					let that = this
					let terOut = setInterval(() => {
						that.postMessageTxt('U点充值',msg)
