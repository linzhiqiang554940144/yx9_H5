<template>
	<view>
		...
		<view class="cu-form-group margin-top">
			<view class="title text-red">备注:最小充值数量为2</view>
		</view>
...
<script>
	export default {
		...
		methods: {
			initData() {
				this.$http.get('/v1/check/recharge/status/mzcz').then(res => {
			...
			initMidform(e) {
				this.$http.get("/v1/recharge/card/limit/id",{params:{pcid: 61}}).then(res => {
					if (res.data.code == 200) {
			...
			initExt() {
				this.$http.get("/v1/recharge/card/discount",{params:{pcid: 61}}).then(res => {
					if (res.data.code == 200) {
			...
			applyTo(e) {
				this.$http.post("/v1/recharge/meizu", {
					num: this.num,
					paypwd: e,
