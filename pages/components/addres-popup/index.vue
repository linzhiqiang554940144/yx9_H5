...
<script>
	...
	export default {
		...
		methods: {
			...
			loadProvinces() {
				this.districtsObj.provinces = address
				this.loadCities(address[0].AreaId)
			},
			loadCities(AreaId) {
				let cities = []
				for(let i = 0; i < address.length;i++) {
					if (address[i].AreaId == AreaId) {
						cities = address[i].children
						break;
					}
				}
				this.districtsObj.cities = cities
				this.loadAreas(cities[0].AreaId)
			},
			loadAreas(AreaId) {
				let areas = []
				for(let i = 0; i < address.length;i++) {
					if (address[i].children) {
						for(let n = 0; n < address[i].children.length;n++) {
							if ( address[i].children[n].AreaId == AreaId) {
								areas = address[i].children[n].children
								break;
							}
						}
					}
				}
				this.districtsObj.areas = areas
			},
			bindChange(event) {
